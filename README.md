...
- **Tradeoffs**: Dependency on an external service. Using a third-party service means diagram DSL traverses external infrastructure; for highly sensitive workloads, consider the self-hosted Puppeteer fallback in `internal/diagram`.
...
> [!IMPORTANT]
> **Secret Handling**: Never commit `.env` files. Ensure keys like `NEAR_AI_API_KEY` and `GCP_API_KEY` are rotated regularly and use least-privilege scopes.
